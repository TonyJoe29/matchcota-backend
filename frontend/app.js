(() => {
  ...
  function loading(title = 'Cargando') {
    setMain(`<div class="card bg-card border-soft shadow-sm"><div class="card-body p-4"><h4 class="text-hard-brown-color">${esc(title)}</h4><p class="text-soft-brown-color mb-0">Cargando informacion...</p></div></div>`);
  }
  ...
  const idByName = (items, name) => {
    const found = items.find((item) => String(item.name).toLowerCase() === String(name || '').toLowerCase());
    return found?.id || '';
  };

  const fieldValue = (value) => esc(value ?? '');

  function petFormPayload(form) {
    const data = clean(formData(form));
    return {
      ...data,
      is_vaccinated: Boolean($('#is_vaccinated')?.checked),
      is_sterilized: Boolean($('#is_sterilized')?.checked),
      compatible_children: Boolean($('#compatible_children')?.checked),
      compatible_dogs: Boolean($('#compatible_dogs')?.checked),
      compatible_cats: Boolean($('#compatible_cats')?.checked)
    };
  }

  function petFormTemplate(cat, pet = null, mode = 'create') {
    const session = getSession();
    const isEdit = mode === 'edit';
    const speciesId = pet ? idByName(cat.species, pet.species) : '';
    const breedId = pet ? idByName(cat.breeds, pet.breed) : '';
    const sizeId = pet ? idByName(cat.sizes, pet.size) : '';
    const cityId = pet ? idByName(cat.cities, pet.city) : '';

    return `
      <div class="d-flex flex-wrap align-items-center mb-4">
        <button class="btn bg-card border-soft text-soft-brown-color rounded-pill shadow-sm mr-3 mb-2 mb-md-0 font-weight-bold" type="button" onclick="window.location.href='mis_mascotas.html'">
      ...
      <form id="pet-form" class="row">
        <div class="col-12 col-lg-9">
          <div class="card bg-card border-soft shadow-sm mb-4" style="border-radius: 15px;">
            <div class="card-body p-4">
              <h6 class="text-orange-color font-weight-bold mb-3 etiqueta-pequena text-uppercase"><i class="fas fa-camera mr-2"></i> Foto</h6>
              <input class="form-control input-custom bg-card border-soft text-soft-brown-color" name="photo_url" type="url" placeholder="URL de la foto" value="${fieldValue(pet?.photo_url)}">
            </div>
          </div>
        ...
        <div class="col-12 col-lg-3">
          <h6 class="text-orange-color font-weight-bold mb-3 etiqueta-pequena text-uppercase"><i class="far fa-eye mr-2"></i> Vista previa</h6>
          <div class="card bg-card border-soft shadow-sm mb-4" style="border-radius: 15px; overflow: hidden;">
            <div class="degradado-rosa-naranja d-flex justify-content-center align-items-center" style="height: 180px;">${pet ? petImg(pet, 'w-100 h-100') : '<i class="fas fa-paw fa-5x text-hard-brown-color"></i>'}</div>
            <div class="card-body"><h6 class="font-weight-bold text-hard-brown-color mb-2">${esc(pet?.name || 'Nueva mascota')}</h6><p class="text-soft-brown-color mb-0" style="font-size:.85rem">${esc(pet?.description || 'Lista para encontrar un hogar.')}</p></div>
          </div>
          <button class="btn bg-orange text-white font-weight-bold btn-block rounded-pill py-3 shadow-sm mb-3" type="submit"><i class="fas fa-save mr-2"></i>${isEdit ? 'Guardar cambios' : 'Publicar mascota'}</button>
          <a class="btn bg-card border-soft text-soft-brown-color font-weight-bold btn-block rounded-pill py-2 shadow-sm" href="mis_mascotas.html">Cancelar</a>
        </div>
      </form>
    `;
  }
  ...
  async function renderPublishPet() {
    ...
    try {
      ...
      $('#pet-form').addEventListener('submit', async (event) => {
        event.preventDefault();
        try {
          const response = await api('/pets', { method: 'POST', body: petFormPayload(event.currentTarget) });
          window.location.href = `pet_detail.html?id=${response.pet.id}`;
        } catch (error) {
          message('#pet-form-message', errorText(error), 'danger');
        }
      });
    } catch (error) {
      errorView('No se pudo abrir el formulario', error);
    }
  }

  async function renderEditPet() {
    const session = requireSession();
    if (!session) return;
    setSidebar('edit_pet.html');
    loading('Cargando mascota');
    try {
      ...
      $('#pet-form').addEventListener('submit', async (event) => {
        event.preventDefault();
        try {
          const response = await api(`/pets/${id}`, { method: 'PUT', body: petFormPayload(event.currentTarget) });
          window.location.href = `pet_detail.html?id=${response.pet.id}`;
        } catch (error) {
          message('#pet-form-message', errorText(error), 'danger');
        }
      ...
      errorView('No se pudo editar la mascota', error);
    }
  }

  async function renderAdoptionForm() {
    const session = requireSession();
    if (!session) return;
    setSidebar('solicitud.html');
      ...
      $('#adoption-form').addEventListener('submit', async (event) => {
        ...
        try {
          await api('/adoptions', {
            ...
            body: {
              pet_id: pet.id,
              motivation: formData(event.currentTarget).motivation,
              home_suitable: $('#home_suitable').checked,
              special_care_experience: $('#special_care_experience').checked
            }
          });
          window.location.href = `solicitud_env.html?pet_id=${pet.id}`;
        } catch (error) {
          message('#adoption-message', errorText(error), 'danger');
        }
      });
    } catch (error) {
      errorView('No se pudo abrir la solicitud', error);
    }
  }
  ...
  async function renderAlerts() {
    ...
    try {
      ...
      errorView('No se pudieron cargar las notificaciones', error);
    }
  }
    ...
    try {
      ...
      $('#profile-form').addEventListener('submit', async (event) => {
        ...
        try {
          ...
          message('#profile-message', errorText(error), 'danger');
        }
      });
    } catch (error) {
    ...
    try {
      ...
      $('#my-pets-list').addEventListener('click', async (event) => {
        ...
        try {
          ...
          message('#my-pets-message', errorText(error), 'danger');
        }
      });
    } catch (error) {
  ...
  async function init() {
    ...
    const routes = {
      ...
      'edit_pet.html': renderEditPet,
      'solicitud.html': renderAdoptionForm,
      'solicitud_env.html': renderAdoptionSent,
      'chat.html': renderChat,
      'profile.html': renderProfile,
      'edit_profile.html': renderEditProfile,
      'configurar_notis.html': renderAlerts,
